...
class AIMatchingService:
    ...
    def _build_model(self):
        ...
        try:
            ...
            user_input = keras.layers.Input(shape=(self.feature_dim,), name='user_features')
            candidate_input = keras.layers.Input(shape=(self.feature_dim,), name='candidate_features')
            
            # 사용자 특성 임베딩
            user_dense = keras.layers.Dense(128, activation='relu')(user_input)
            user_dense = keras.layers.Dropout(0.3)(user_dense)
            user_dense = keras.layers.Dense(64, activation='relu')(user_dense)
            
            # 후보자 특성 임베딩
            candidate_dense = keras.layers.Dense(128, activation='relu')(candidate_input)
            candidate_dense = keras.layers.Dropout(0.3)(candidate_dense)
            candidate_dense = keras.layers.Dense(64, activation='relu')(candidate_dense)
            
            # 특성 결합 및 호환성 계산
            concat = keras.layers.Concatenate()([user_dense, candidate_dense])
            ...
            # 출력: 호환성 점수 (0-1)
            output = keras.layers.Dense(1, activation='sigmoid', name='compatibility')(dense3)
